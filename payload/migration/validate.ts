...
async function main() {
    ...
    const tagCounts = tally(artworks.flatMap(a => a.tags))
    ...
    console.log(`\ntags (${tagCounts.size} distinct across ${artworks.filter(a => a.tags.length > 0).length} tagged records):`)
    console.log(`  ${format(tagCounts)}`)
    console.log(`\nmaterial: ${withMaterial.length} records carry a description, ${new Set(withMaterial.map(a => a.material)).size} distinct`)
